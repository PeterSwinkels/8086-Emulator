...
Public Module MSDOSModule
   ...
   Private Function GetRelocationItemPositions(Executable As List(Of Byte)) As RelocationItemsStr
      Try
         ...
         With RelocationItems
            .Start = GetWord(Executable, MSDOSHeaderE.RelocationTableOffset)
            .Count = GetWord(Executable, MSDOSHeaderE.RelocationCount)
            .Ending = GetWord(Executable, MSDOSHeaderE.HeaderSize) << &H4%

            Position = .Start
            For RelocationItem As Integer = &H0% To .Count - &H1%
               .Positions.Add(.Ending + GetWord(Executable, Position) + (GetWord(Executable, Position + &H2%) << &H4%))
               Position += &H4%
            Next RelocationItem
         End With
   ...
   'This procedure returns the word at the specified address in the specified data.
   ...
   Public Function LoadExecutable(FileName As String) As Boolean
      Try
         ...
         If GetWord(Executable, MSDOSHeaderE.Signature) = SIGNATURE Then
            CPU.Registers(CPU8086Class.SegmentRegistersE.CS, NewValue:=CS + GetWord(Executable, MSDOSHeaderE.CodeSegment))
            CPU.Registers(CPU8086Class.Registers16BitE.IP, NewValue:=GetWord(Executable, MSDOSHeaderE.InstructionPointer))
            CPU.Registers(CPU8086Class.SegmentRegistersE.ES, NewValue:=CPU.Registers(CPU8086Class.SegmentRegistersE.DS))
            CPU.Registers(CPU8086Class.SegmentRegistersE.SS, NewValue:=CS + GetWord(Executable, MSDOSHeaderE.StackSegment))
            CPU.Registers(CPU8086Class.Registers16BitE.SP, NewValue:=GetWord(Executable, MSDOSHeaderE.StackPointer))
            ...
            With GetRelocationItemPositions(Executable)
               For Each RelocationItemPosition As Integer In .Positions
                  PutWord(Executable, RelocationItemPosition, GetWord(Executable, RelocationItemPosition) + IP)
                  PutWord(Executable, RelocationItemPosition + &H2%, GetWord(Executable, RelocationItemPosition + &H2%) + CS)
